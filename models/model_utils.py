...
def merge_norm_linear_layer(layer, mean, std):
    """Merge the normalization and linear layer into a single layer.

    Args:
        layer: The linear layer.
        mean: The mean of the normalization.
        std: The std of the normalization.

    Returns:
        A new linear layer with the merged weights and bias.
    """
    mean_tensor = torch.tensor(mean, dtype=layer.weight.dtype).cuda()
    std_tensor = torch.tensor(std, dtype=layer.weight.dtype).cuda()
    coef_tensor = layer.weight.data / std_tensor
    bias_tensor = layer.bias.data - (mean_tensor / std_tensor) * layer.weight.data
    coef_param = torch.nn.parameter.Parameter(coef_tensor)
    bias_param = torch.nn.parameter.Parameter(bias_tensor)
    layer.weight = coef_param
    layer.bias = bias_param
...
##########################################################################################
"""
Reference: https://github.com/y0ast/DUE/blob/main/due/layers/spectral_norm_fc.py
"""
